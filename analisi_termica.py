...
def scarica_ephemeris(comando, centro, t_start, t_stop, step):
    url = f"https://ssd.jpl.nasa.gov/api/horizons.api?format=text&COMMAND='{comando}'&EPHEM_TYPE=VECTORS&CENTER='{centro}'&OUT_UNITS='KM-S'&START_TIME='{t_start}'&STOP_TIME='{t_stop}'&STEP_SIZE='{step}'&VEC_TABLE='2'"
    testo = requests.get(url).text
    try:
        dati = testo[testo.index("$$SOE")+5 : testo.index("$$EOE")]
        matches = re.findall(r"X =\s*([-\d\.E\+]+)\s*Y =\s*([-\d\.E\+]+)\s*Z =\s*([-\d\.E\+]+)\s*VX =\s*([-\d\.E\+]+)\s*VY =\s*([-\d\.E\+]+)\s*VZ =\s*([-\d\.E\+]+)", dati)
