...
// Costanti termodinamiche pure (non presenti nell'header astronomico)
...
// ==========================================
// FUNZIONE PRINCIPALE: SOLUTORE ODE
// ==========================================
void solve_thermal_ode(
    ...
    double* out_Temp_history // [num_steps * num_nodes]
) {
    // Array delle proprietà fisiche
    ...
    // Matrice delle conduttanze (1D array finto 2D)
    double* G_cond = (double*)calloc(num_nodes * num_nodes, sizeof(double));

    // 1. INIZIALIZZAZIONE PROPRIETÀ NODI
    for (int i = 0; i < num_nodes; i++) {
        T_current[i] = 20.0 + 273.15; // 20°C iniziali
        Q_int[i] = 0.0;
        
        // Calcolo centro e area
        int nv = num_verts[i];
        ...
        if (nv == 4) { // Semplificazione area rettangolare
        ...
        } else { area = 3.14159; } // HGA
        Area[i] = area;

        // Assegnazione Masse e Calore Specifico (da MATLAB)
        if (types[i] == 1) { // Bus
            alpha[i] = 0.15; eps[i] = 0.05; C_cap[i] = (2700 * area * 0.05) * 897;
        } else if (types[i] == 2) { // SA
            alpha[i] = 0.70; eps[i] = 0.85; C_cap[i] = (2000 * area * 0.005) * 700;
        } else if (types[i] == 3) { // HGA
            alpha[i] = 0.20; eps[i] = 0.85; C_cap[i] = (1600 * area * 0.01) * 1100;
        } else { // Batterie (Type 4 o 5)
            alpha[i] = 0.0; eps[i] = 0.0; C_cap[i] = (18.0 / 6.0) * 900.0; // 3kg per faccia
        }
    }

    // Aggiungi 5W di dissipazione interna al Bus (come in MATLAB)
    for (int i=0; i<num_nodes; i++) { if (types[i] == 1) { Q_int[i] = 0.5; } } // Spalmato sui nodi bus

    // 2. MATRICE CONDUTTANZE (G_cond) semplificata rispetto a MATLAB
    for (int i = 0; i < num_nodes; i++) {
        for (int j = 0; j < num_nodes; j++) {
            if (i == j) continue;
            double dist = norm3(centers[i*3]-centers[j*3], centers[i*3+1]-centers[j*3+1], centers[i*3+2]-centers[j*3+2]);
            
            if (types[i] == 1 && types[j] == 1 && dist < 1.2) G_cond[i*num_nodes+j] = 5.0; // Bus-Bus
            if (types[i] == 2 && types[j] == 2 && dist < 1.5) G_cond[i*num_nodes+j] = 3.0; // SA-SA
            if ((types[i] == 1 && types[j] == 2) && dist < 1.0) G_cond[i*num_nodes+j] = 0.5; // Bus-SA
            if (types[i] >= 4 && types[j] == 1 && dist < 2.0) G_cond[i*num_nodes+j] = 5.0; // Bat-Bus
            if (types[i] == types[j] && types[i] >= 4 && dist < 1.0) G_cond[i*num_nodes+j] = 15.0; // Bat-Bat (stessa batteria)
        }
    }

    // 3. LOOP DI INTEGRAZIONE FISICA (SOSTITUTO DI ODE45)
    for (int step = 0; step < num_steps; step++) {
        
        double s_flux = S_flux[step];
        int ecl = is_eclipse[step];
        
        // --- Assetto MAVEN (DCM: Body to Inertial) ---
        ...
        double Yb[3]; cross3(Zb[0], Zb[1], Zb[2], Xb[0], Xb[1], Xb[2], Yb);
        
        // --- Calcolo Derivate Termiche (dT/dt) ---
        for (int i = 0; i < num_nodes; i++) {
            double q_sol = 0, q_rad = 0, q_cond = 0, q_ex = Q_int[i];

            if (types[i] >= 4) { // Batteria
                double I_bat = ecl ? 2.67 : 12.0; // Scarica o Carica
                q_ex += (I_bat * I_bat * 0.016) / 6.0; // I2R
                
                // Riscaldatore Proporzionale
